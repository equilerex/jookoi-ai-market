...
- **`track`** in `@for`: use a stable id (`track item.id`). `track item` is correct for primitive items (strings, numbers). For object items it is fine only if identity survives refetches, which it usually doesn't. Check the collection's type before flagging it. `track $index` is fine for static lists and wrong for lists that insert, remove, or reorder.
