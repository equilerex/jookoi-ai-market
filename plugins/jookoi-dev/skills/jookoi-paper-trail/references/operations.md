...
Each one is a rule about *when not to write*, as much as how to write.
...
3. `_architecture/` at the repo root. For the working set, run `list` rather than reading files.
...
## Staleness — `jookoi-paper-trail stale [days]`

Two checks. `now` items whose `ts_touched` is `days` (default 14) or more old are listed as candidates to `park` or `drop`. This works on the private layer too, since it reads stored dates rather than commits.

Context files are compared on their `updated:` line against the folder's last commit date:
...
Reporting only, deliberately. When context contradicts code, the code wins and the context is corrected *at the point of noticing*. That correction is judgement about what changed and why, which a batch job does not have.

Zero context files is a clean result, not an error.
