...
```
items.json  ──flush──▶  archive/items-YYYY-MM.json
now / parked / done / dropped     done and dropped items only
```

Nothing live moves. `now` and `parked` items stay in `items.json` through a flush, so there is no "move it somewhere first" step.
...
2. Append them to `archive/items-<this-month>.json`, keyed by ID. An ID already in that file is refused.
3. Remove them from `items.json` and set `last_flush`.

Model-owned, before calling flush: the classification step in `references/flush-prompt.md`. Durable content in a done item is promoted to `ARCHITECTURE.md` or a decision record first, since the archive is not read back by default.
...
`items.json` is plain JSON with a schema in `references/store-format.md`. By hand: edit the item, keep IDs unique and `next_id` above every ID ever issued, set `ts_touched`. Move finished items into the month's archive file. Do this only when Node is genuinely unavailable.
